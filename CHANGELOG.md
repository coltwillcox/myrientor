...
### Changed
- `getDirectoryListingRec` and `shouldDownload` now retry up to `downloadMaxRetries` (3) times on connection or HTTP errors, consistent with `downloadFile`

## [0.13.0] - 2026-03-07

### Added
- Final summary panel printed after all devices finish, showing total files downloaded/skipped/deleted/errors, bytes downloaded/skipped/total, elapsed time, and devices synced

### Changed
- `devicePanel` now reuses `panelTopLabeled` instead of duplicating the border logic

## [0.12.1] - 2026-03-06

### Fixed
- ETA now uses the 10-second sliding window speed instead of the cumulative average since start, giving a more accurate and responsive estimate
